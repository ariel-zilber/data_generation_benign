...
[[ -z pass ]] && { echo "!!! MariaDB root password not set. Check the Vagrant file."; exit 1; }
...
sudo debconf-set-selections <<< "maria-db-$MARIADB_VERSION mysql-server/root_password password pass"
sudo debconf-set-selections <<< "maria-db-$MARIADB_VERSION mysql-server/root_password_again password pass"
...
Q1="GRANT ALL ON *.* TO 'root'@'%' IDENTIFIED BY 'pass' WITH GRANT OPTION;"
Q2="FLUSH PRIVILEGES;"
SQL="${Q1}${Q2}"
